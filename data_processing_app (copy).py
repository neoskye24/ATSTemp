    ...
    if operation == "Data Processing":
        ...
        if region == "US":
            ...
            if dfs_to_concat:
                ...
                st.info(f"Using modified data from: {modified_files[0]}")
                # else:
                #     # If no modified data, check Merge Final India
                #     merge_dir = os.path.join('database', 'Merge Final India')
                #     if os.path.exists(merge_dir):
                #         merged_files = sorted([
                #             f for f in os.listdir(merge_dir)
                ...
                #             st.info(
                #                 f"Using merged data from: {merged_files[0]}")
                #         else:
                #             india_df = final_dataframe_india
                #             st.warning(
                #                 "No data files found in Merge Final India directory, So fetching your first upload"
                #             )
                #     else:
                #         india_df = pd.DataFrame()
                #         st.error("Merge Final India directory not found")
            # else:
            #     # If Modified Data directory doesn't exist, check Merge Final India
            #     merge_dir = os.path.join('database', 'Merge Final India')
            #     if os.path.exists(merge_dir):
            #         merged_files = sorted([
            ...
            #             india_df = pd.read_csv(latest_file)
            #             st.info(f"Using merged data from: {merged_files[0]}")
            #         else:
            #             india_df = pd.DataFrame()
            #             st.warning(
            #                 "No data files found in Merge Final India directory"
            #             )
            #     else:
            #         india_df = pd.DataFrame()
            #         st.error("No data directories found in database folder")
                    ...
                    if selected_index is not None:
                        ...
                        if region == "US":
                            ...
                            with col3:
                                ...
                                recruiter_options = ["Sumanth", "Dhan"]  # Add more names as needed
                                current_recruiter = selected_candidate.get('Recruiter', '')
                                recruiter = st.selectbox(
                                    "Recruiter",
                                    options=recruiter_options,
                                    index=recruiter_options.index(current_recruiter)
                                    if current_recruiter in recruiter_options else 0)
                        ...
                        with col1:
                            if st.button("Save Changes"):
                                try:
                                    ...
                                    df.loc[selected_index, 'Recruiter'] = recruiter
