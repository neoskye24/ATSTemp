...
def preprocess_linkedin_india(df):
    ...
    existing_cols_to_remove = [
        col for col in processed_df.columns if col in columns_to_remove
    ]
    ...
    if 'first_name' in processed_df.columns and 'last_name' in processed_df.columns:
        processed_df['name'] = processed_df['first_name'].fillna(
            '') + ' ' + processed_df['last_name'].fillna('')
    ...
    expected_cols = [
        'first_name', 'last_name', 'location', 'current_title',
        'current_company', 'email', 'phone', 'profile_url', 'active_project',
        'notes'
    ]

    # Check if columns are unnamed (like "Unnamed: 0", "Unnamed: 1", etc.)
    unnamed_pattern = any(
        col.startswith('Unnamed:') for col in processed_df.columns)

    if unnamed_pattern:
        print(
            "Detected unnamed columns - applying standard LinkedIn column names"
        )
        ...
        if rename_map:
            ...
            print(
                f"  Renamed columns according to expected LinkedIn structure")
    ...
    if 'active_project' in processed_df.columns:
        split_results = processed_df['active_project'].apply(
            extract_parentheses)
        ...
        print(
            "Split active_project column - extracted content in parentheses to 'status' column"
        )
    ...
    cols_to_remove = [
        col for col in processed_df.columns if col in ['Feedback', 'feedback']
    ]
    if cols_to_remove:
